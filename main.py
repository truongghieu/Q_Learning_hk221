...
for i in range(9):
    ...
    for eps in range(EPISODES):
        ...
        while not done:
            ...
            if done:
                # finish
                print("LR : "+ str(round(LEARNING_RATE,2))  +" Episodes : ",eps)
...
for i in range(9):
    plt.plot(reward_list[i],label = "LR : "+ str(round(0.1*(i+1),2)))
plt.xlabel("Episode")
plt.ylabel("Reward")
...
plt.savefig("fig/Reward.png",bbox_inches='tight')

# plt.savefig("rewardEachEps.png")
# del rewardEachEps
plt.clf()
for i in range(9):
    plt.plot(action_list[i],label = "LR : "+ str(round(0.1*(i+1),2)))
...
plt.legend()
plt.savefig("fig/Action.png",bbox_inches='tight')

# save number action each learning rate 
minAct = []
for i in range(9):
    minAct.append(min(action_list[i]))
f = open("fig/minAction.txt","w")
f.write(str(minAct))
f.close()
plt.clf()
...
for i in range(9):
    ...
    plt.xlabel("Episode")
    plt.ylabel("Number Action")
    plt.title("MountainCar-v0 LR : "+ str(round(0.1*(i+1),2)))
    plt.savefig("fig/Action_"+str(i)+".png",bbox_inches='tight')
    plt.clf()
    plt.plot(reward_list[i])
    plt.xlabel("Episode")
    plt.ylabel("Reward")
    plt.title("MountainCar-v0 LR : "+ str(round(0.1*(i+1),2)))
    plt.savefig("fig/Reward_"+str(i)+".png",bbox_inches='tight')
    plt.clf()